...
export default function RecruiterJobs() {
  const [jobs, setJobs] = useState(mockJobs.filter((j) => j.company === 'TechNova Solutions' || j.company === 'DataStream Inc.'));
        ...
        {jobs.map((job) => (
          <div key={job.id} className="card card-interactive" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{
              ...
              <button
                onClick={() => alert(`Opening analytics/candidate reviews for Job ID: ${job.id}`)}
